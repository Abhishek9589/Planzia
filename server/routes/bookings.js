...
router.get('/owner', authenticateToken, async (req, res) => {
  try {
    ...
    const withVenue = bookings.map(b => ({
      ...b,
      venue_id: b.venue_id?._id?.toString() || b.venue_id,
...
// Cancel booking as customer (protected)
router.post('/:id/cancel', authenticateToken, async (req, res) => {
  try {
    ...
    const venue = await Venue.findById(booking.venue_id).lean();
    if (venue) {
      const owner = await User.findById(venue.owner_id, { name: 1, email: 1 }).lean();
      if (owner && owner.email) {
        try {
          const cancelNotificationHtml = `
            <!DOCTYPE html>
            <html lang="en">
            <head>
              <meta charset="UTF-8">
                  ...
                  <td style="padding: 0;">
                    ...
                    <div style="border: 1px solid #d0d7de; border-radius: 6px; padding: 20px; margin: 0 0 32px 0; background-color: #f6f8fa;">
                      ...
                      <div style="margin: 0;">
                        ...
                        <p style="color: #424a52; margin: 0 0 8px 0; font-size: 13px; font-weight: 500;">Customer:</p>
                        <p style="color: #1a1a1a; margin: 0 0 16px 0; font-size: 14px; font-weight: 400;">${booking.customer_name}</p>
                        <p style="color: #424a52; margin: 0 0 8px 0; font-size: 13px; font-weight: 500;">Event Date:</p>
                        <p style="color: #1a1a1a; margin: 0; font-size: 14px; font-weight: 400;">${new Date(booking.event_date).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                      </div>
                    </div>
                    <p style="color: #1a1a1a; margin: 0 0 4px 0; font-size: 13px; font-weight: 400; line-height: 1.5;">Thanks,</p>
                    <p style="color: #1a1a1a; margin: 0 0 32px 0; font-size: 13px; font-weight: 400; line-height: 1.5;">The Planzia Team</p>
                    <p style="color: #666666; margin: 0; font-size: 12px; font-weight: 400; line-height: 1.6;">Your venue is now available for other bookings.</p>
                  </td>
                </tr>
              </table>
            </body>
          ...
          await transporter.sendMail({
            from: { name: 'Planzia', address: process.env.EMAIL_USER },
            to: owner.email,
            subject: `Booking Cancelled - ${booking.venue_name}`,
            html: cancelNotificationHtml
          });
        } catch (notificationError) {
          console.error('Error sending cancellation notification to owner:', notificationError);
        }
      }
    }

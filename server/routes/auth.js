...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    if (state) {
      try { const s = JSON.parse(decodeURIComponent(state)); userType = ['customer', 'venue-owner'].includes(s.userType) ? s.userType : 'customer'; } catch {}
    }
    if (error || !code) {
      return res.send(`
        <script>
          if (window.opener && !window.opener.closed) {
            try {
              window.opener.postMessage({ type: 'GOOGLE_AUTH_ERROR', error: 'oauth_failed' }, '${process.env.CLIENT_URL}');
              setTimeout(() => window.close(), 100);
            } catch (error) { window.location.href = '${process.env.CLIENT_URL}/signin?error=oauth_failed'; }
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({ client_id: process.env.GOOGLE_CLIENT_ID, client_secret: process.env.GOOGLE_CLIENT_SECRET, code, grant_type: 'authorization_code', redirect_uri: process.env.GOOGLE_REDIRECT_URI })
    });
    ...
    res.send(`
      <script>
        if (window.opener && !window.opener.closed) {
          try {
            window.opener.postMessage({ type: 'GOOGLE_AUTH_SUCCESS', accessToken: '${accessToken}', refreshToken: '${refreshToken}' }, '${process.env.CLIENT_URL}');
            setTimeout(() => { window.close(); }, 100);
          } catch (error) { window.location.href='${process.env.CLIENT_URL}/?access_token=${accessToken}&refresh_token=${refreshToken}'; }

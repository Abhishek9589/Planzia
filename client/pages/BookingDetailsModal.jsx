...
export default function BookingDetailsModal({
  ...
  const calculateNumberOfDays = () => {
    // First, check if dates_timings array exists and has multiple entries
    ...
    // Fall back to number_of_days field if it exists
  ...
  // If customerDetailsOnly mode, show comprehensive modal with all booking details
  if (customerDetailsOnly) {
    const numberOfDays = calculateNumberOfDays();
    const pricePerDay = venueDetails?.price_per_day || booking.price_per_day || 0;

    // Calculate forward: venue amount → platform fee → GST → total
            ...
            <div className="border-b border-gray-200 pb-4">
              ...
              <div className="space-y-2">
                {booking.dates_timings.map((dateiming, idx) => {
                  ...
                  // Format date - handle both string and Date object formats
                  let dateStr = 'N/A';
                  try {
                    const dateObj = new Date(dateiming.date);
                    dateStr = dateObj.toLocaleDateString('en-IN', {
